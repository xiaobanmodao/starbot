...
**Windows 桌面 AI 自动化 Agent — Discord / Web UI / 桌面客户端三端控制**

Starbot 是一个运行在本地 Windows 机器上的 AI 自动化代理，通过 Discord、Web UI 或桌面客户端接收自然语言指令，使用大语言模型（LLM）推理，并调用 50+ 内置工具完成各类桌面自动化任务——鼠标点击、键盘输入、文件操作、网页抓取、屏幕识别等，同时具备长期记忆、操作回滚、后台任务和可扩展技能插件能力。

> An AI-powered Windows desktop automation agent with Discord, Web UI, and native desktop clients.
...
> Desktop 桌面客户端（原生窗口包装 Web UI）：`python start_desktop.py`
...
| 🧩 **技能插件** | 内置 15+ 技能 + 外部 SKILL.md 生态，支持在线安装/更新 |
| 🖥️ **多端控制** | Discord / Web UI / 桌面客户端（pywebview）/ Tk GUI 四种入口 |
...
|------|------|
| `web_search` | 搜索引擎搜索 |
| `web_research` | 深度网络调研（多轮搜索+抓取+交叉验证） |
...
| `learn_video` | 学习视频：自动字幕优先，失败自动 Whisper 转写 |
| `learn_url` | 学习网页内容并记忆 |
...
内置技能位于 `skills/` 目录，随项目一起分发。用户额外安装的技能位于 `skills_ext/`，不包含在仓库中。

| 技能 | 工具 | 用途 |
|------|------|------|
| **weather** | `get_weather` | 天气查询（wttr.in，无需 API Key） |
| **math_tools** | `calculate`, `convert_unit`, `statistics_calc`, `number_base_convert` | 数学计算、单位换算、统计、进制转换 |
| **dev_tools** | `format_json`, `encode_decode`, `hash_text`, `timestamp_convert`, `color_convert`, `text_diff`, `regex_test`, `generate_uuid` | 开发者工具箱（JSON 格式化、编解码、哈希、时间戳、颜色转换、正则测试等） |
| **translate** | `translate`, `detect_language`, `batch_translate`, `list_languages` | 多语言翻译（MyMemory API） |
| **reminder** | `set_reminder`, `list_reminders`, `cancel_reminder`, `snooze_reminder` | 定时提醒（Windows 桌面通知） |
| **pomodoro** | `start_pomodoro`, `pomodoro_status`, `cancel_pomodoro` | 番茄钟计时器 |
| **currency** | `convert_currency`, `list_exchange_rates` | 实时汇率换算（open.er-api.com） |
| **stock** | `get_stock_price`, `search_stock`, `stock_history`, `market_overview` | 股票/ETF/指数行情（Yahoo Finance） |
| **news** | `get_headlines`, `fetch_rss`, `hacker_news`, `search_news` | 新闻聚合（RSS + Hacker News） |
| **qrcode_skill** | `generate_qr`, `generate_wifi_qr` | 二维码生成 |
| **smart_search** | `wiki_lookup`, `scrape_table`, `crawl_site`, `compare_sources` | 深度搜索（Wikipedia、表格提取、站点爬取、多源对比） |
| **sysmon** | `system_stats`, `top_processes`, `disk_usage`, `network_stats` | 系统资源监控（psutil） |
| **browser_agent** | `browser_open`, `browser_navigate`, `browser_click`, `browser_type`, `browser_wait`, `browser_screenshot`, `browser_get_text`, `browser_fill_form`, `browser_close` | Playwright 浏览器自动化 |
| **monitor** | `watch_stock`, `watch_webpage`, `watch_condition`, `list_monitors`, `cancel_monitor` | 后台条件监控（价格/网页变化，Discord 告警） |
| **self_learn** | `knowledge_map`, `analyze_topic`, `distill_knowledge`, `set_learning_goal`, `review_knowledge` | 知识管理与 LLM 蒸馏 |
| **video_learn** | `video_info`, `learn_video_plus`, `learn_playlist`, `batch_learn_videos` | 视频学习（字幕提取 + Whisper 转写，支持 YouTube/B站等） |
| **example** | `echo`, `get_timestamp` | 示例技能（开发参考） |

> 💡 用户通过 `/skill install` 安装的外部技能存放在 `skills_ext/`，不会被 git 跟踪。
...
# 配置环境
cp .env.template .env
# 编辑 .env 填写配置（或启动时自动进入配置向导）

# 启动（Discord 模式，自动检查依赖）
python start.py
...
> 也可通过环境变量 `STARBOT_INSTALLER=uv` 或 `STARBOT_INSTALLER=auto` 让启动脚本优先使用 uv 安装依赖。
...
├── start.py               # 启动检查器（自动安装依赖、Discord 入口）
├── start_gui.py           # GUI 客户端入口（Tk）
├── start_webui.py         # Web UI 客户端入口（浏览器）
├── start_desktop.py       # 桌面客户端入口（pywebview 原生窗口）
├── config.py              # 配置管理（单例 _Config、.env 读写）
├── config_wizard.py       # 独立交互式配置向导（TUI）
├── core/
│   ├── brain.py           # LLM 推理引擎（ReAct 循环、上下文压缩、token 追踪）
│   ├── adapter.py         # LLM API 适配器（OpenAI 兼容封装）
│   ├── session_controller.py # 线程化会话控制器（GUI/Web UI 共用）
│   ├── task_manager.py    # 后台多任务并发管理
│   ├── skill_manager.py   # 技能插件系统（Python 原生 + SKILL.md 外部生态）
│   ├── op_log.py          # 操作日志与文件回滚
│   └── notifier.py        # 后台任务 Discord 通知桥接
├── actions/
│   ├── executor.py        # 工具实现层（50+ 工具）
│   ├── input_win32.py     # Win32 低级输入 API（鼠标/键盘/中文）
│   └── web_helpers.py     # 网页抓取辅助（trafilatura）
├── comms/
│   ├── discord_client.py  # Discord Bot 客户端（流式回复、斜杠命令、确认按钮）
│   ├── local_service.py   # 本地客户端统一 API 层（GUI/Web 共用）
│   ├── webui_server.py    # HTTP 服务器（REST API + 静态文件）
│   ├── gui_client.py      # Tk GUI 客户端
│   └── text_safety.py     # 输出过滤（数字垃圾检测）
├── memory/
│   └── store.py           # SQLite + FTS5 trigram 记忆存储
├── skills/                # 内置技能插件
│   ├── browser_agent.py   # Playwright 浏览器自动化
│   ├── video_learn.py     # 视频学习（字幕提取 + Whisper 转写）
│   ├── smart_search.py    # 深度网络调研
│   ├── self_learn.py      # 自主学习
│   ├── weather.py         # 天气查询
│   ├── stock.py           # 股票行情
│   ├── news.py            # 新闻聚合
...
│   ├── monitor.py         # 网站/服务监控
│   ├── sysmon.py          # 系统资源监控
│   ├── qrcode_skill.py    # 二维码生成
│   ├── math_tools.py      # 数学计算
│   └── dev_tools.py       # 开发辅助工具
├── webui/                 # Web UI 前端（HTML/CSS/JS）
│   ├── index.html
│   ├── styles.css
│   └── app.js
└── tools/
    └── changelog_release.py # 发布辅助脚本
```
...
```
用户输入（三端任选其一）
├── Discord 消息 → StarBotClient.on_message()
├── Web UI / Desktop → WebUiServer → LocalClientService
└── GUI (Tk) → gui_client → LocalClientService
    ↓
SessionController（线程化会话管理）
    ↓
Brain（LLM 推理） ← Memory.recall（多关键词搜索相关记忆）
    ↓                ← SkillManager（技能推荐注入）
Tool Calling（原生 function calling 或文本解析回退）
    ↓
[危险工具确认] → Discord: ConfirmView 按钮 / Web UI: 前端确认
    ↓ (确认通过)
Executor（执行工具） → op_log.backup_file（写入/删除前备份）
    ↓
[ReAct 注入] 失败时注入重试提示，最多3次
    ↓
结果 → Discord 进度卡片 / Web UI 事件流 + Memory.save()
```
